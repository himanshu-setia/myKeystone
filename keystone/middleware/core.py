...
class AuthContextMiddleware(wsgi.Middleware):
    ...
    def process_request(self, request):
        ...
        if AUTH_TOKEN_HEADER in request.headers:
            composite_token = request.headers.get(AUTH_TOKEN_HEADER).strip()
            if len(composite_token) < 65:
                if request.path != '/v3/token-auth' and request.path != '/v3/token-auth-ex' and request.path != '/token-auth' and request.path != '/token-auth-ex' and request.path != '/v3/get-presigned-url-token' and request.path != '/get-presigned-url-token':
