...
@dependency.requires('assignment_api', 'credential_api', 'account_config_api',
...
class Manager(manager.Manager):
    ...
    def create_project(self, tenant_id, tenant, initiator=None):
        ...
        if tenant.get('parent_id') is not None:
            ...
            for ref in parents_list:
                if ref.get('account_id') != tenant.get('account_id'):
                    raise exception.ForbiddenAction(
                        action=_('cannot create a project within a different '
                                 'account than its parents.'))
    ...
    def assert_account_enabled(self, account_id, account=None):
        """Assert the Account is enabled.

        :raise AssertionError if account is disabled.
        """
        if account is None:
            account = self.get_account(account_id)
        if not account.get('enabled', True):
            raise AssertionError(_('Account is disabled: %s') % account_id)

    def assert_account_not_federated(self, account_id, account):
        """Assert the Account's name and id do not match the reserved keyword.
        ...
        :raise AssertionError if account named match the value in the config.

        """
        # NOTE(marek-denis): We cannot create this attribute in the __init__ as
        # config values are always initialized to default value.
        federated_account = (CONF.federation.federated_account_name or
                            federation.FEDERATED_ACCOUNT_KEYWORD).lower()
        if (account.get('name') and account['name'].lower() == federated_account):
            raise AssertionError(_('Account cannot be named %s')
                                 % federated_account)
        if (account_id.lower() == federated_account):
            raise AssertionError(_('Account cannot have ID %s')
                                 % federated_account)

    def assert_project_enabled(self, project_id, project=None):
        """Assert the project is enabled and its associated account is enabled.

        :raise AssertionError if the project or account is disabled.
        """
        if project is None:
            project = self.get_project(project_id)
        self.assert_account_enabled(account_id=project['account_id'])
    ...
    def create_account(self, account_id, account, initiator=None):
        if (not self.identity_api.multiple_accounts_supported and
                account_id != CONF.identity.default_account_id):
            raise exception.Forbidden(_('Multiple accounts are not supported'))
        self.assert_account_not_federated(account_id, account)
        account.setdefault('enabled', True)
        account['enabled'] = clean.account_enabled(account['enabled'])
        ret = self.driver.create_account(account_id, account)

        notifications.Audit.created(self._ACCOUNT, account_id, initiator)

        if MEMOIZE.should_cache(ret):
            self.get_account.set(ret, self, account_id)
            self.get_account_by_name.set(ret, self, ret['name'])
        return ret

    def duplicate(self, account_id):
        return self.driver.duplicate(account_id)

    @manager.response_truncated
    def list_accounts(self, hints=None):
        return self.driver.list_accounts(hints or driver_hints.Hints())

    @notifications.disabled(_ACCOUNT, public=False)
    def _disable_account(self, account_id):
        """Emit a notification to the callback system account is been disabled.

        This method, and associated callback listeners, removes the need for
        making direct calls to other managers to take action (e.g. revoking
        account scoped tokens) when a account is disabled.
    ...
    def update_account(self, account_id, account, initiator=None):
        ...
        #     enabled=False
        if (original_account.get('enabled', True) and
                not account.get('enabled', True)):
            notifications.Audit.disabled(self._ACCOUNT, account_id, initiator,
                                         public=False)

        self.get_account.invalidate(self, account_id)
        self.get_account_by_name.invalidate(self, original_account['name'])
        return ret
    ...
    def _delete_account_contents(self, account_id):
        ...
        Before we delete a account, we need to remove all the entities
        ...
        def _delete_projects(project, projects, examined):
            ...
            try:
                ...
                LOG.debug(('Project %(projectid)s not found when '
                           'deleting account contents for %(accountid)s, '
                           'continuing with cleanup.'),
                          {'projectid': project['id'],
                           'accountid': account_id})

        proj_refs = self.list_projects_in_account(account_id)
    ...
    # NOTE(henry-nash): list_projects_in_account is actually an internal method
    # and not exposed via the API.  Therefore there is no need to support
    # driver hints for it.
    def list_projects_in_account(self, account_id):
        return self.driver.list_projects_in_account(account_id)
    ...
    def get_project_by_name(self, tenant_name, account_id):
        return self.driver.get_project_by_name(tenant_name, account_id)
...
class Driver(object):
    ...
    def list_accounts(self, hints):
        ...
        :returns: a list of account_refs or an empty list.

        """
        raise exception.NotImplemented()  # pragma: no cover

    @abc.abstractmethod
    def list_accounts_from_ids(self, account_ids):
        """List accounts for the provided list of ids.

        :param account_ids: list of ids

        :returns: a list of account_refs.

        This method is used internally by the assignment manager to bulk read
        a set of accounts given their ids.
    ...
    def get_account(self, account_id):
        ...
        :returns: account_ref
        :raises: keystone.exception.AccountNotFound
    ...
    def list_project_ids_from_account_ids(self, account_ids):
        """List project ids for the provided list of account ids.

        :param account_ids: list of account ids

        :returns: a list of project ids owned by the specified account ids.

        This method is used internally by the assignment manager to bulk read
        a set of project ids given a list of account ids.
    ...
    def list_projects_in_account(self, account_id):
        ...
        :param account_id: the driver MUST only return projects
                          within this account.
    ...
    # Account management functions for backends that only allow a single
    # account.  Currently, this is only LDAP, but might be used by other
    # backends in the future.
    def _set_default_account(self, ref):
        """If the account ID has not been set, set it to the default."""
        if isinstance(ref, dict):
            ...
            return [self._set_default_account(x) for x in ref]
        else:
            raise ValueError(_('Expected dict or list: %s') % type(ref))

    def _validate_default_account(self, ref):
        """Validate that either the default account or nothing is specified.

        Also removes the account from the ref so that LDAP doesn't have to
        persist the attribute.

        """
        ref = ref.copy()
        account_id = ref.pop('account_id', CONF.identity.default_account_id)
        self._validate_default_account_id(account_id)
        return ref

    def _validate_default_account_id(self, account_id):
        """Validate that the account ID specified belongs to the default account.
...
MEMOIZE_CONFIG = cache.get_memoization_decorator(section='account_config')


@dependency.provider('account_config_api')
class AccountConfigManager(manager.Manager):
    """Default pivot point for the Account Config backend."""
    ...
    # exception.
    #
    # Only those options that affect the account-specific driver support in
    ...
    def __init__(self):
        super(AccountConfigManager, self).__init__(CONF.account_config.driver)
    ...
    def _assert_valid_config(self, config):
        ...
        if not config:
            raise exception.InvalidAccountConfig(
            ...
            if (not config[group] or not
                ...
                msg = _('The value of group %(group)s specified in the '
                        'config should be a dictionary of options') % {
                            'group': group}
                raise exception.InvalidAccountConfig(reason=msg)
    ...
    def _assert_valid_group_and_option(self, group, option):
        ...
        if not group and not option:
            ...
            # are operating on all config options for that account.
            return
        ...
        if not group and option:
            ...
            msg = _('Option %(option)s found with no group specified while '
                    'checking account configuration request') % {
                        'option': option}
        ...
        if (group and group not in self.whitelisted_options and
            ...
            msg = _('Group %(group)s is not supported '
                    'for account specific configurations') % {'group': group}
            raise exception.InvalidAccountConfig(reason=msg)
        ...
        if option:
            if (option not in self.whitelisted_options[group] and option not in
                ...
                msg = _('Option %(option)s in group %(group)s is not '
                        'supported for account specific configurations') % {
    ...
    def create_config(self, account_id, config):
        """Create config for a account

        :param account_id: the account in question
        ...
        :raises keystone.exception.InvalidAccountConfig: when the config
        ...
        self.delete_config_options(account_id)
        self.delete_config_options(account_id, sensitive=True)
        # ...and create the new one
        for option in whitelisted:
            self.create_config_option(
                account_id, option['group'], option['option'], option['value'])
        ...
        # cache.
        self.get_config_with_sensitive_info.invalidate(self, account_id)
        return self._list_to_config(whitelisted)

    def get_config(self, account_id, group=None, option=None):
        """Get config, or partial config, for a account
        ...
        :raises keystone.exception.AccountConfigNotFound: when no config found
                that matches account_id, group and option specified
        :raises keystone.exception.InvalidAccountConfig: when the config
        ...
        """
        self._assert_valid_group_and_option(group, option)
        whitelisted = self.list_config_options(account_id, group, option)
        if whitelisted:
    ...
    def update_config(self, account_id, config, group=None, option=None):
        """Update config, or partial config, for a account
        ...
        config for this account, updating or creating new options if these did
        ...
        """
        def _assert_valid_update(account_id, config, group=None, option=None):
            ...
            if group:
                ...
                # a AccountConfigNotFound
                if not self._get_config_with_sensitive_info(account_id,
                                                            group, option):
                    if option:
                        msg = _('option %(option)s in group %(group)s') % {
                            'group': group, 'option': option}
        ...
        def _update_or_create(account_id, option, sensitive):
            """Update the option, if it doesn't exist then create it."""

            try:
                self.create_config_option(
                    account_id, option['group'], option['option'],
        ...
        _assert_valid_update(account_id, update_config, group, option)

        whitelisted, sensitive = self._config_to_list(update_config)

        for new_option in whitelisted:
            _update_or_create(account_id, new_option, sensitive=False)
        for new_option in sensitive:
            _update_or_create(account_id, new_option, sensitive=True)

        self.get_config_with_sensitive_info.invalidate(self, account_id)
        return self.get_config(account_id)

    def delete_config(self, account_id, group=None, option=None):
        """Delete config, or partial config, for the account.

        :param account_id: the account in question
        :param group: an optional specific group of options
        :param option: an optional specific option within the group

        If group and option are None, then the entire config for the account
        ...
        :raises keystone.exception.InvalidAccountConfig: when group/option
        ...
        if group:
            ...
            current_config = self._get_config_with_sensitive_info(account_id)
        ...
        self.delete_config_options(account_id, group, option)
        self.delete_config_options(account_id, group, option, sensitive=True)
        self.get_config_with_sensitive_info.invalidate(self, account_id)

    def _get_config_with_sensitive_info(self, account_id, group=None,
                                        option=None):
        """Get config for a account/group/option with sensitive info included.
        ...
        """
        whitelisted = self.list_config_options(account_id, group, option)
        sensitive = self.list_config_options(account_id, group, option,
                                             sensitive=True)
        ...
        for each_whitelisted in whitelisted:
            ...
            try:
                ...
                warning_msg = _LW(
                    ...
                    'substitution reference - account: %(account)s, group: '
                ...
                warning_msg = _LW(
                    ...
                    'config option substitution reference - account: '
                    '%(account)s, group: %(group)s, option: %(option)s, '
                    'value: %(value)s.')
    ...
    @MEMOIZE_CONFIG
    def get_config_with_sensitive_info(self, account_id):
        """Get config for a account with sensitive info included.

        This method is not exposed via the public API, but is used by the
        identity manager to initialize a account with the fully formed config
        options.

        """
        return self._get_config_with_sensitive_info(account_id)


@six.add_metaclass(abc.ABCMeta)
class AccountConfigDriver(object):
    """Interface description for a Account Config driver."""

    @abc.abstractmethod
    def create_config_option(self, account_id, group, option, value,
                             sensitive=False):
        """Creates a config option for a account.

        :param account_id: the account for this option
    ...
    def get_config_option(self, account_id, group, option, sensitive=False):
        """Gets the config option for a account.
        ...
        :returns: dict containing group, option and value
        :raises: keystone.exception.AccountConfigNotFound: the option doesn't
                                                          exist.
    ...
    @abc.abstractmethod
    def list_config_options(self, account_id, group=None, option=False,
                            sensitive=False):
        """Gets a config options for a account.
    ...
    def update_config_option(self, account_id, group, option, value,
                             sensitive=False):
        """Updates a config option for a account.
    ...
    def delete_config_options(self, account_id, group=None, option=None,
                              sensitive=False):
        """Deletes config options for a account.

        Allows deletion of all options for a account, all options in a group
