...
class SqlIDMapping(test_backend_sql.SqlTests):
    ...
    def load_sample_data(self):
        ...
        accountA = {'id': uuid.uuid4().hex, 'name': uuid.uuid4().hex}
        self.accountA = self.resource_api.create_account(accountA['id'], accountA)
        accountB = {'id': uuid.uuid4().hex, 'name': uuid.uuid4().hex}
        self.accountB = self.resource_api.create_account(accountB['id'], accountB)

    def clean_sample_data(self):
        if hasattr(self, 'accountA'):
            self.accountA['enabled'] = False
            self.resource_api.update_account(self.accountA['id'], self.accountA)
            self.resource_api.delete_account(self.accountA['id'])
        if hasattr(self, 'accountB'):
            self.accountB['enabled'] = False
            self.resource_api.update_account(self.accountB['id'], self.accountB)
            self.resource_api.delete_account(self.accountB['id'])
    ...
    def test_id_mapping_crud(self):
        ...
        local_entity1 = {'account_id': self.accountA['id'],
                         'local_id': local_id1,
                         'entity_type': mapping.EntityType.USER}
        local_entity2 = {'account_id': self.accountB['id'],
                         'local_id': local_id2,
        ...
        self.assertEqual(self.accountA['id'], local_id_ref['account_id'])
        ...
        self.assertEqual(self.accountB['id'], local_id_ref['account_id'])
        ...
        public_id3 = self.id_mapping_api.create_id_mapping(
            {'account_id': self.accountB['id'], 'local_id': local_id2,
             'entity_type': mapping.EntityType.USER},
    ...
    def test_id_mapping_handles_unicode(self):
        initial_mappings = len(mapping_sql.list_id_mappings())
        local_id = u'fäké1'
        local_entity = {'account_id': self.accountA['id'],
                        'local_id': local_id,
                        'entity_type': mapping.EntityType.USER}
    ...
    def test_purge_mappings(self):
        ...
        # Create five mappings,two in accountA, three in accountB
        self.id_mapping_api.create_id_mapping(
            {'account_id': self.accountA['id'], 'local_id': local_id1,
             'entity_type': mapping.EntityType.USER})
        self.id_mapping_api.create_id_mapping(
            {'account_id': self.accountA['id'], 'local_id': local_id2,
             'entity_type': mapping.EntityType.USER})
        public_id3 = self.id_mapping_api.create_id_mapping(
            {'account_id': self.accountB['id'], 'local_id': local_id3,
             'entity_type': mapping.EntityType.GROUP})
        public_id4 = self.id_mapping_api.create_id_mapping(
            {'account_id': self.accountB['id'], 'local_id': local_id4,
             'entity_type': mapping.EntityType.USER})
        public_id5 = self.id_mapping_api.create_id_mapping(
            {'account_id': self.accountB['id'], 'local_id': local_id5,
             'entity_type': mapping.EntityType.USER})

        self.assertThat(mapping_sql.list_id_mappings(),
                        matchers.HasLength(initial_mappings + 5))

        # Purge mappings for accountA, should be left with those in B
